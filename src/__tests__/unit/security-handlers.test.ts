...
describe('Security MSW Handlers', () => {
  it('GET /api/security/overview returns 200 with correct values', async () => {
    ...
    expect(data).toHaveProperty('cameraCount', 264)
    expect(data).toHaveProperty('accessDeviceCount', 38)
    expect(data).toHaveProperty('todayAlerts', 7)
    expect(data).toHaveProperty('todayVisitors', 41)
  })
  ...
  it('GET /api/security/access returns 200 with points and abnormal records', async () => {
    ...
    expect(res.status).toBe(200)
    const data = await res.json()
    expect(data).toHaveProperty('todayTotal', 5832)
