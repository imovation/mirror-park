...
export default function OverviewTopMetrics() {
  ...
  return (
    <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
      <TopMetricsCard icon="map" label="占地面积" value={`${formatNumber(data.landArea ?? 0)}㎡`} />
      <TopMetricsCard icon="building" label="建筑面积" value={`${formatNumber(data.buildingArea ?? 0)}㎡`} />
      <TopMetricsCard icon="group" label="班级数" value={formatNumber(data.classCount ?? 0)} />
      <TopMetricsCard icon="block" label="建筑栋数" value={formatNumber(data.buildingCount ?? 0)} />
      <TopMetricsCard icon="teacher" label="教职工" value={formatNumber(data.totalTeachers ?? 0)} />
      <TopMetricsCard icon="student" label="学生数" value={formatNumber(data.totalStudents ?? 0)} />
    </div>
  )
}
