  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6, flex: 1, minHeight: 0 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, justifyContent: 'center', flexShrink: 0 }}>
        <div style={{ fontSize: 'var(--font-size-xxl)', fontWeight: 700, color: 'var(--text-primary)' }}>{data.totalStudents}</div>
        <div style={{ fontSize: 'var(--font-size-sm)', color: 'var(--text-muted)' }}>在校学生</div>
        <div style={{ width: 1, height: 20, background: 'var(--border-light)' }} />
        <span style={{ color: 'var(--color-chart-8)', fontSize: 'var(--font-size-md)', fontWeight: 600 }}>♂ {Math.round(data.maleRatio * 100)}%</span>
        <span style={{ color: 'var(--color-chart-5)', fontSize: 'var(--font-size-md)', fontWeight: 600 }}>♀ {Math.round(data.femaleRatio * 100)}%</span>
      </div>
        ...
        {data.grades.map((g) => (
          ...
          >
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: 12, fontSize: 'var(--font-size-md)' }}>
              <span style={{ color: 'var(--color-chart-8)', fontWeight: 700 }}>♂ {g.male}</span>
